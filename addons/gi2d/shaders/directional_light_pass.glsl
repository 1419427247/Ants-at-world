...
layout(push_constant, std430) uniform UniformParameters {
    ...
    float max_distance;        // 最大搜索距离（世界单位）
...
float directional_light_visibility(
    ...
    for (int i = 0; i < uniform_parameters.max_steps; i++) {
        ...
        if (step_dist < surface_threshold) {
            ...
            float t = total_distance / uv_max_distance;
...
void main() {
    ...
    // 宽高比修正
    vec2 aspect_vec = vec2(1.0, luminance_size_vec.x / luminance_size_vec.y);
    float uv_max_distance = uniform_parameters.max_distance * uniform_parameters.world_to_uv.x;
    ...
    for (int j = 0; j < pcf; j++) {
        ...
        float vis = directional_light_visibility(
            jittered_uv, origin_height, luminance_size_vec, min_step_uv, uv_max_distance, bd
        );
    ...
    imageStore(output_image, coordinates, vec4(visibility, blocker_distance / uv_max_distance, 0.0, 1.0));
}
