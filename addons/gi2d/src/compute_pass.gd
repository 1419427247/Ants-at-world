...
## 获取 GI 相机有效 zoom 值（当 source_viewport 是 GILightViewport 时）。
## 用于世界空间 ↔ UV 空间坐标转换。
## 返回的 vec2 = (zoom.x, zoom.y)，非 GILightViewport 时返回 (1,1)。
func _get_gi_zoom() -> Vector2:
	if source_viewport and source_viewport is GILightViewport:
		var gi_vp := source_viewport as GILightViewport
		if gi_vp.camera:
			return gi_vp.camera.zoom
	return Vector2(1.0, 1.0)

## 计算 world_to_uv 缩放因子（每个世界单位对应多少 UV 单位）。
## world_to_uv = zoom / viewport_size
## 用于 GDScript 层将世界单位参数转换为 UV 空间传入 shader。
func _get_world_to_uv() -> Vector2:
	var zoom := _get_gi_zoom()
	var vp_w := maxf(source_viewport.size.x if source_viewport else 1.0, 1.0)
	var vp_h := maxf(source_viewport.size.y if source_viewport else 1.0, 1.0)
	return Vector2(zoom.x / vp_w, zoom.y / vp_h)

## 返回公共片元信息（FragmentInfo）的 8 字节编码。
## 子类可在 _get_push_data() 中 prepend 此数据作为前 8 字节，
## 无需重复调用 _get_world_to_uv()。
func _get_fragment_info_prefix() -> PackedByteArray:
	var buf := PackedByteArray()
	buf.resize(8)
	var w2u := _get_world_to_uv()
	buf.encode_float(0, w2u.x)
	buf.encode_float(4, w2u.y)
	return buf
...
func _process(delta: float) -> void:
	...
	if output_dimensions.x != _output_width or output_dimensions.y != _output_height or output_format != _output_format or not output_texture_resource_id.is_valid():
		# 必须先释放 uniform set（它引用了输出纹理），再释放输出纹理
		# 否则输出纹理先释放后 uniform set 中的引用失效，导致 free_rid 报错
