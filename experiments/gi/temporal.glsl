...
void main() {
    ...
    vec4 current_pixel = imageLoad(current_frame_image, coordinates);
    vec4 history_pixel = imageLoad(history_image, coordinates);
    ...
    for (int offset_y = -1; offset_y <= 1; offset_y++) {
        for (int offset_x = -1; offset_x <= 1; offset_x++) {
            ...
            vec3 neighbor_color = imageLoad(current_frame_image, neighbor_coordinates).rgb;
