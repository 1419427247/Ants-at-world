...
// binding 2: 有向距离场（RGBA16F: R=到最近异质点距离, GB=方向, A=1，作为深度引导）
...
void main() {
    ...
    // B3 样条小波权重（1D 核，距离 0/1/2 对应权重 c0/c1/c2）
    const float c0 = 1.0;
    ...
    vec3 color_sum = center_pixel.rgb * c0;
    float weight_sum = c0;

    int step = uniform_parameters.step_size;

    // 1D 滤波：仅沿指定方向采样 5 个点（可分离优化，2.5× 加速）
    for (int offset = -2; offset <= 2; offset++) {
        if (offset == 0) continue;

        ivec2 sample_coordinates = coordinates + (
            uniform_parameters.direction == 0
                ? ivec2(offset * step, 0)
                : ivec2(0, offset * step)
        );
        sample_coordinates = clamp(sample_coordinates, ivec2(0), texture_size - 1);

        vec2 sample_uv = (vec2(sample_coordinates) + 0.5) / vec2(texture_size);
        vec4 sample_pixel = texture(input_image, sample_uv);

        // 空间权重（1D B3 小波）
        float weight_spatial = (abs(offset) == 1) ? c1 : c2;
